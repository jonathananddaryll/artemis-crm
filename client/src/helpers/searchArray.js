...
export default function SearchArray( searchString, searchArray, searchType ) {
  ...
  let trimmed = searchString.split("").filter((element, index, array) => {
      ...
      const searchWords = ( contactsArray, arrayOfWords ) => {
        for(let contact = 0; contact < contactsArray.length; contact++){
          for(let name = 0; name < arrayOfWords.length; name++){
            ...
            if(includesTLC(contactsArray[contact].first_name, arrayOfWords[name])){
              if(includesTLC(contactsArray[contact].last_name, arrayOfWords[name])){
                results.push(contact)
              }else{
                results.push(contact)
              }
            }else if(includesTLC(contactsArray[contact].last_name, arrayOfWords[name])){
                results.push(contact)
            }
          }
        }
